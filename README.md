...
이 프로젝트는 **전북대학교 연구실 안전관리시스템** 의 일상 점검 절차를 **자동화**하기 위한 Python 스크립트입니다.  
GitHub Actions를 이용해 **매주 월요일 오전 8시(한국시간)** 자동 실행되며, 연구실 안전 점검 항목을 자동으로 클릭 및 저장합니다.
...
2. GitHub Actions (`.github/workflows/auto_inspection.yml`)  
   - 매주 월요일 오전 8시(KST)에 자동 실행 (`cron: "0 23 * * *"`)
...
├── .github/
│ └── workflows/
│ └── daily-check.yml # GitHub Actions 워크플로 설정
└── README.md
```
